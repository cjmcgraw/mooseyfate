...
class ChunkyKNNHypothesis(Hypothesis):

    def __init__(self, k, window=99):
        super(ChunkyKNNHypothesis, self).__init__()
        self._window = window
    ...
    def update(self, vector, attacked, outcome):
        super(ChunkyKNNHypothesis, self).update(vector, attacked, outcome)
        if outcome == 1:
            result = True
        else:
            result = False
